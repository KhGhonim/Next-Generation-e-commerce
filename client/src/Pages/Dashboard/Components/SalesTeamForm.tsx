...
const defaultMember: Omit<SalesTeamMember, "_id"> = {
  name: "",
  email: "",
  phone: "",
  role: "",
...
function SalesTeamForm({ member, onSubmit, onCancel }: SalesTeamFormProps) {
  const [formData, setFormData] = useState<Omit<SalesTeamMember, "_id">>(defaultMember);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = member?._id
        ? `${API_URL}/api/sales-team/${member._id}`
        : `${API_URL}/api/sales-team`;
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6 bg-gradient-to-br from-white to-zinc-50/30">
            ...
            <div className="flex justify-end gap-4 pt-6 border-t border-zinc-200 bg-white/50 -mx-6 px-6 pb-6">
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.02, y: -1 }}
                whileTap={{ scale: 0.98 }}
                aria-label={member ? "Update team member" : "Create team member"}
              >
